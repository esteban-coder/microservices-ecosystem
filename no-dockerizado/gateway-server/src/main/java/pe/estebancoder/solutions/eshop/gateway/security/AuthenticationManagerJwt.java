...
public class AuthenticationManagerJwt implements ReactiveAuthenticationManager{
	...
	private final Logger logger = LoggerFactory.getLogger(AuthenticationManagerJwt.class);
	...
	public Mono<Authentication> authenticate(Authentication authentication) {
		return Mono.just(authentication.getCredentials().toString())
				.map(token -> {
					...
					//String llave = Base64.getEncoder().encodeToString(llaveJwt.getBytes());
					//String llave = "no es la cadena";
					logger.info("------------ llave" + llave);
